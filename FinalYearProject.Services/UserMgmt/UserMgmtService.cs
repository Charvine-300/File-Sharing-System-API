...
) : IUserMgmtService
{
    ...
    CancellationToken cancellationToken)
    {
        ...
        if (parameters.AttributeIds != null && parameters.AttributeIds.Any())
        {
            query = query.Where(user =>
                parameters.AttributeIds.All(attributeId =>
                    user.UsersAttributes.Any(userAttribute =>
                        userAttribute.AttributeId == attributeId
                    )
                )
            );
        }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                To = user.Email!,
                Subject = "Welcome to Vault - Your Account Has Been Created",
    ...
    CancellationToken cancellationToken)
    {
        try
        {
            ...
            var existingAttributes = await database.Users_Attributes
                .Where(x => x.UserId == user.Id)
                .ToListAsync(cancellationToken);

            database.Users_Attributes.RemoveRange(existingAttributes);


            var newAttributes = attributes.Select(attribute => new UserAttribute
            {
                UserId = user.Id,
                AttributeId = attribute.Id,
                CreatedAt = DateTimeOffset.UtcNow
            ...
            await database.Users_Attributes.AddRangeAsync(
                newAttributes,
                cancellationToken);
